...
sample_output_file = sys.argv[2]
n_sample = int(sys.argv[3])
update_step = int(sys.argv[4])
...
def sample(fname, n_points, lower_bounds=None, upper_bounds=None, n_update=100, n_secondary=0):
    ...
    while pt_count < n_points:
        ...
        objective = QuadExpr((target-reaction)*(target-reaction)/square_ranges[ind]) # added
...
samplez = sample(input_file, n_sample, n_update=update_step, n_secondary = 33)
model = read(input_file)
head = str(model.getVars())
n = len(head)
head = head[1:(n-1)]
np.savetxt(sample_output_file,samplez,delimiter=",",header=head)
